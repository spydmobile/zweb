type CallbackFunctionDocReady = (...args: any[]) => void;
import { tentSensorMap, houseSensorMap } from './tools/toolslib'
import { z } from "zod";
import * as Highcharts from 'highcharts'
import { bgColorPlugin, bgImagePlugin } from './charts'
 ...
 */
var sanitizeHTML = function (str:string) {
...
const unitStatus = z.object({
  ...
  TempC: z.number(),
  RH: z.number(),
  Notes: z.string().or(z.number()).optional(),
});
...
function docReady(fn: CallbackFunctionDocReady) {
...
function processData(data24: any[]) {
  return new Promise(async (resolve, reject) => {
    ...
    for (const nid of nidList) {
      let current24 = await uniqueArray.filter(r => r.nid == nid)
      let dataset = await current24.sort(({ date: a }, { date: b }) => b - a);
...
docReady(function () {
  ...
  voxButton?.addEventListener("click", function () {
    ...
    recognition.onnomatch = (event: Event) => {
      //@ts-ignore
  ...
  fetch("data24.json")
    ...
    .then(async (raw) => {
      finalStats = await processData(raw)
    ...
    .then(() => {
      ...
      fetch("data.json")
        ...
        .then(tentData => {
          ...
          if (pageSpan !== null) pageSpan.innerHTML = 'Page: ' + new Date().toLocaleString()
          if (buildSpan !== null) buildSpan.innerHTML = 'Build: ' + buildDate
          ...
          if (tentDisplay !== null) {
            ...
            for (const record of data) {
              const name = tentSensorMap(record.nid)
              if (name !== false) {
        ...
        class="w3-button w3-display-topright w3-round-large w3-hover-red"><b>&times;</b></span>
          ...
          if (houseDisplay !== null) {
            ...
            list.classList.add('w3-ul')
            for (const record of data) {
              const name = houseSensorMap(record.nid)
              if (name !== false) {
                const item = document.createElement('li')
                //  item.classList.add('list-group-item')
        ...
        class="w3-button w3-display-topright w3-round-large w3-hover-red"><b>&times;</b></span>
          ...
          const tomatoData = data.filter((record: any) => record.nid === 11)[0]
          tomatoData.Name = tentSensorMap(tomatoData.nid)
          console.log("finalStats",finalStats);
          const tomatoStats = finalStats[tomatoData.nid]
          tomatoStats.sort((a: any, b: any) => a.date - b.date);
          console.debug({ tomatoData, tomatoStats });

          const woodThermostatData = data.filter((record: any) => record.nid === 5)[0]
          woodThermostatData.Name = tentSensorMap(woodThermostatData.nid)
          const woodPlenumData = data.filter((record: any) => record.nid === 7)[0]
          woodPlenumData.Name = tentSensorMap(woodPlenumData.nid)
          const woodThermostatStats = finalStats[woodThermostatData.nid]
          woodThermostatStats.sort((a: any, b: any) => a.date - b.date);
          const woodPlenumStats = finalStats[woodPlenumData.nid]
          woodPlenumStats.sort((a: any, b: any) => a.date - b.date);
          ...
          if (tomatoDisplay && homeDisplay) {
            ...
            if (houseTomatoTemp) houseTomatoTemp.innerText = tomatoData.TempC + "C"
            ...
            if (houseWoodThermostatValue) houseWoodThermostatValue.innerText = woodThermostatData.TempC + "C"
            ...
            if (houseWoodPlenumTemp) houseWoodPlenumTemp.innerText = woodPlenumData.TempC + "C"
        ...
        .then(() => {
          ...
          modals.forEach(element => {
            ...
            element.addEventListener('show.bs.modal', function (event: any) {
              ...
              const modal: any = document.getElementById(`staticBackdrop${nid}`);
              console.log('nid', nid);
              // filter the stats
              ...
              var tempData = res.map((r: any) => { return [r.date, r.TempC] })
              var humData = res.map((r: any) => { return [r.date, r.RH] })
              Highcharts.chart({
                chart: {
                  renderTo: 'chart' + nid,
                  type: 'spline',
            ...
            element.addEventListener('goStats', function (event: any) {
...
const buildChartFromData = async (canvasId: string, chartTitle: string, data: any, options: any) => {
